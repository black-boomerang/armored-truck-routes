...
def get_data(data_path=None) -> Tuple[pd.DataFrame, np.ndarray]:
    """ Загрузка данных о терминалах и времени между ними """

    if data_path is None:
        data_path = DATA_ROOT

    terminal_data_path = os.path.join(
        data_path, 'terminal_data_hackathon v4.xlsx')
    ...
    terminals = points.set_index('TID').join(
        incomes.set_index('TID')).reset_index()
    ...
    times['Origin_index'] = times['Origin_tid'].apply(
        lambda tid: tid_to_ind[tid])
    times['Destination_index'] = times['Destination_tid'].apply(
        lambda tid: tid_to_ind[tid])
